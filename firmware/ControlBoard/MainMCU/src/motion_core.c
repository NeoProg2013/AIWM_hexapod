...
void motion_core_start_motion(const motion_t* motion, const motion_config_t* motion_config) {
    ...
    if (g_current_motion.is_ignore_ext_speed) {
        servo_driver_set_speed(g_current_motion.speed);
    } else {
        float speed = (float)g_current_motion.speed / 100.0f * (float)motion_config->speed;
        servo_driver_set_speed((int32_t)speed);
    }
}
...
void motion_core_process(void) {
    ...
    if (!is_motion_completed) {
        ...
        g_current_motion.motion_time += 20;//g_current_motion.time_step;
        if (g_current_motion.motion_time == g_current_motion.time_update) {
            ...
            if (g_current_motion.is_ignore_ext_speed) {
                ...
                float speed = (float)g_current_motion.speed / 100.0f * (float)g_current_motion_config.speed;
                servo_driver_set_speed((int32_t)speed);
            }
        }
